use super::snapshot_helpers::{cli_snapshot_settings, run_constraint_command};
...
fn test_validate_no_constraints() {
    ...
    cmd.current_dir(&temp_dir).arg("validate");
...
fn test_validate_structural_only() {
    ...
    add_cmd1
        ...
        .arg("security")
        .arg("--text")
        .arg("Test with verification")
        .arg("--author")
        .arg("test")
        .arg("--verification")
        .arg("echo 'test passed'");
    ...
    cmd.assert()
        ...
        .stdout(predicate::str::contains("Structural validation passed"))
        .stdout(predicate::str::contains("Valid: 2"))
        .stdout(predicate::str::contains(
            "All constraints are structurally valid",
        ))
        .stdout(predicate::str::contains(
            "Use --execute to run verification commands",
        ));
}
...
fn test_validate_with_execute_success() {
    ...
    add_cmd
        ...
        .arg("test")
        .arg("--text")
        .arg("Constraint with successful verification")
        .arg("--author")
        .arg("test")
        .arg("--verification")
        .arg("echo 'verification successful' && exit 0");
    add_cmd.assert().success();

    let mut cmd = constraint_command();
    cmd.current_dir(&temp_dir).arg("validate").arg("--execute");
...
fn test_validate_with_execute_failure() {
    ...
    add_cmd
        ...
        .arg("test")
        .arg("--text")
        .arg("Constraint with failing verification")
        .arg("--author")
        .arg("test")
        .arg("--verification")
        .arg("echo 'verification failed' && exit 1");
    add_cmd.assert().success();
...
fn test_validate_category_filter() {
    ...
    add_cmd1
        ...
        .arg("security")
        .arg("--text")
        .arg("Security constraint")
        .arg("--author")
        .arg("test")
        .arg("--verification")
        .arg("echo 'security ok'");
    add_cmd1.assert().success();
    ...
    cmd.assert()
        ...
        .stdout(predicate::str::is_not(predicate::str::contains(
            "Testing constraint",
        )));
}
...
fn test_validate_specific_constraint() {
    ...
    let id_line = stdout
        .lines()
        .find(|line| line.contains("Constraint added with ID:"))
        .unwrap();
...
#[test]
fn test_validate_structural_invalid_constraint() {
    let temp_dir = TempDir::new().unwrap();
    ...
    add_cmd1
        ...
        .arg("security")
        .arg("--text")
        .arg("Valid constraint")
        .arg("--author")
        .arg("test");
    add_cmd1.assert().success();

    // Manually create an invalid constraint file (bad category format)
    let constraints_dir = temp_dir.path().join(".newton/constraints");
    let invalid_constraint = r#"{
        "version": 1,
        "id": "nt-123456",
        "type": "MUST",
    ...
    fs::write(constraints_dir.join("nt-123456.jsonl"), invalid_constraint).unwrap();

    let mut cmd = constraint_command();
    cmd.current_dir(&temp_dir).arg("validate");

    cmd.assert()
        .failure() // Should exit with code 1 due to invalid constraint
        .stdout(predicate::str::contains("Validating 2 constraint(s)"))
        .stdout(predicate::str::contains("VALID"))
        .stdout(predicate::str::contains("INVALID"))
        .stdout(predicate::str::contains("Valid: 1"))
        .stdout(predicate::str::contains("Invalid: 1"))
        .stdout(predicate::str::contains("Some constraints are invalid"));
}
...
fn test_validate_verbose_output() {
    ...
    // Add constraint with verification
    let mut add_cmd = constraint_command();
    add_cmd
        .current_dir(&temp_dir)
        .arg("add")
        ...
        .arg("test")
        .arg("--text")
        .arg("Constraint with verbose output")
        .arg("--author")
        .arg("test")
        .arg("--verification")
        .arg("echo 'verbose test output'");
    add_cmd.assert().success();
    ...
    cmd.assert()
        ...
        .stdout(predicate::str::contains("Duration:")) // Verbose mode shows duration
        .stdout(predicate::str::contains(
            "All validations completed successfully",
        ));
}
